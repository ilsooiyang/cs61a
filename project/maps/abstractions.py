...
def user_reviewed_restaurants(user, restaurants):
    ...
    """
    names = list(user_reviews(user)) #list of restaurant names reviewed by user
    return [r for r in restaurants if
     restaurant_name(r) in names] #list of restaurants reviewed by user
...
def restaurant_mean_rating(restaurant):
    ...
    return False if not ratings else mean(ratings)
    # END Question 2
