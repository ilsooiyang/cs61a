...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        pairs.append([min(centroids, key=lambda c: distance(restaurant_location(r), c)), r])
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        r_clusters = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(c) for c in r_clusters]
        n += 1
    return centroids
    #each iteration raises the accuracy of centroid coordinates
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([(x - mean_x) * (y - mean_y) for x,y in xy])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictors = [find_predictor(user, reviewed, feature_fn)
     for feature_fn in feature_fns]

    return max(predictors, key=lambda l: l[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            di[restaurant_name(r)] = user_rating(user, restaurant_name(r))
        else:
            di[restaurant_name(r)] = predictor(r)
    return di
    # END Question 9
