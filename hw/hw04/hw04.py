...
def div_interval(x, y):
    ...
    AssertionError
    """
    assert 0 not in range(lower_bound(y), upper_bound(y)), "cannot divide by an interval that spans zero"
...
def quadratic(x, a, b, c):
    ...
    points = [lower, upper, ext_y]

    if ext_x >= lower_bound(x) and ext_x <= upper_bound(x):
        return [min(points), max(points)]
    else:
        return [min(points[:2]), max(points[:2])]
