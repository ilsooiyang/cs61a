...
def replace_elem(lst, index, elem):
    ...
    new = lst[:]
    return new[:index] + [elem] + new[index+1:]
...
def put_piece(board, max_rows, column, player):
    ...
    """
    row = max_rows - 1
    if get_piece(board, 0, column) != '-': #test if the top row is empty
        return -1, board
    else:
        while row >= 0:
            new_board = board[:]
            if get_piece(new_board, row, column) == '-':
                new_row = replace_elem(new_board[row], column, player)
                new_board = replace_elem(new_board, row, new_row)
